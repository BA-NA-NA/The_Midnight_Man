...
function OnPreRender () {
	...
	if (capture && DNRequire) {
		...
		DepthCam.camera.pixelRect = Rect(0,0,Screen.width,Screen.height);
		DepthCam.camera.Render();
		DNBuffer.ReadPixels(Rect(0,0,Screen.width,Screen.height), 0, 0);
		DNBuffer.Apply();
	}
	if (capture) {
		...
		dom.camera.aspect = asp;
		dom.camera.pixelRect = Rect(0,0,Screen.width,Screen.height);
		dom.camera.Render();
		RT.ReadPixels(Rect(camera.pixelRect.x,camera.pixelRect.y,Screen.width,Screen.height), 0, 0);
		RT.Apply();
	}
}
